...
class PatternMaker:
  ...
  def makeMCirclesPattern(self):
    dot_color = "GhostWhite"
    spacing = self.square_size
    r = spacing / self.radius_rate
    for x in range(1,self.cols+1):
      for y in range(1, self.rows+1):
        if (x == 1 and y == self.rows):
          dot = SVG("circle", cx=x * spacing, cy=y * spacing, r=(1.25*r), stroke="white", fill="white")
          self.g.append(dot)
          minidot = SVG("circle", cx=x*spacing, cy=y*spacing, r = (0.1*r), stroke=dot_color, fill=dot_color)
          self.g.append(minidot)
        else:
          dot = SVG("circle", cx=x * spacing, cy=y * spacing, r=r, stroke="white", fill="white")
          self.g.append(dot)
          minidot = SVG("circle", cx=x*spacing, cy=y * spacing, r = (0.1*r), stroke=dot_color, fill=dot_color)
          self.g.append(minidot)
