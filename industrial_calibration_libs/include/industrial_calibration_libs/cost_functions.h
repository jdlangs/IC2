...
struct ResearchIntrinsicCF3
{
  ResearchIntrinsicCF3(const double observed_x, const double observed_y,
    Point3D point, int num_images, int image_number) : observed_x_(observed_x), 
    observed_y_(observed_y), point_(point), 
    num_images_(num_images), image_number_(image_number) { }
  ...
  const T* const target_pose, T* residual) const
  {
    ...
    const T* target_angle_axis(&target_pose[image_number_*6 + 0]);
    const T* target_position(&target_pose[image_number_*6 + 3]);
  ...
  // client code.
  static ceres::CostFunction *Create(const double observed_x, const double observed_y,
    Point3D point, const int num_images, const int image_number)
  {
    // int n = num_images*6;
    // return (new ceres::AutoDiffCostFunction<ResearchIntrinsicCF3, 2, 9, ceres::DYNAMIC>(new 
    //   ResearchIntrinsicCF3(observed_x, observed_y, point, num_images, image_number), n));
    return (new ceres::AutoDiffCostFunction<ResearchIntrinsicCF3, 2, 9, 91*6>(new 
      ResearchIntrinsicCF3(observed_x, observed_y, point, num_images, image_number)));    
  }
